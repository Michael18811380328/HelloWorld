...
上面这种写法跟传统的面向对象语言（ C++ 和 Java）差异很大。ES6 提供了更接近传统语言的写法，引入了 Class（类）这个概念，作为对象的模板。通过`class`关键字，可以定义类。
...
里面有一个`constructor`方法，这就是构造方法，而`this`关键字则代表实例对象。也就是说，ES5 的构造函数`Point`，对应 ES6 的`Point`类的构造方法。
...
ES6 的类，完全可以看作构造函数的另一种写法。类的数据类型就是函数，类本身就指向构造函数。使用的时候，也是直接对类使用`new`命令，跟构造函数的用法完全一致。
...
另外，==类的内部所有定义的方法，都是不可枚举的==（non-enumerable）。ES5 的构造函数定义的方法，就是可枚举的。
...
上面代码中，定义了一个空的类`Point`，会自动为它添加一个空的`constructor`方法。
...
与 ES5 一样，实例的属性除非显式定义在其本身（即定义在实例对象上，即`this`），否则都是定义在原型对象上（即定义在`class`上）。
...
上面代码中，`prop`属性有对应的存值函数和取值函数，因此赋值和读取行为都被自定义了。存值函数和取值函数是设置在属性的 Descriptor 对象上的。
...
由于本质上，ES6 的类只是 ES5 的构造函数的一层包装，所以函数的许多特性都被`Class`继承，包括`name`属性。name 属性总是返回紧跟在`class`关键字后面的类名。
...
还有一种解决方法是使用`Proxy`，获取方法的时候，自动绑定`this`。这种方法很少使用
...
上面的代码，一眼就能看出，`foo`类有两个实例属性，写起来也比较简洁。
