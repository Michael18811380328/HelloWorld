...
说明1：HEAD表示最近一个提交
说明2：提交ID可以在git log中找到(github可视化界面也可以找到)
说明3：--hard 表示删除commit信息，删除源码，本地代码也删除
--mixed(默认方式) 删除commit信息，删除index信息，不会删除源码。可以编辑后，git add git commit
--soft 删除commit，不会删除index信息，不会删除源码。可以直接commit
~~~
